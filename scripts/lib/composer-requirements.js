...
function generateBaseComposerRequirements(config) {
  ...
  if (isDir(`${config.paths.local.themes}/planet4-master-theme`)) {
    composer(
      'remove --no-update greenpeace/planet4-master-theme',
  ...
  if (baseComposerConfig?.repositories) {
    for (const k in baseComposerConfig.repositories) {
      if (
        baseComposerConfig.repositories[k]?.type === 'package' &&
				baseComposerConfig.repositories[k]?.package.name.startsWith(
				  'plugins/'
				)
      ) {
        baseComposerConfig.repositories[k].package.type =
					'wordpress-plugin';
      }
    }
  }
...
function generateNROComposerRequirements(config) {
  ...
  const merged = {
    require: {
      ...baseComposerConfig.require,
      ...nroComposerConfig.require,
    },
    config: {...baseComposerConfig.config, ...nroComposerConfig.config},
    scripts: {
      ...baseComposerConfig.scripts,
      ...nroComposerConfig.scripts,
    },
  };
